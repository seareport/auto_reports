...
def plot_map_availability(ioc_detided, ax=None, bbox: shapely.box = None):
    ...
    colors = np.array(
        [plt.colormaps["tab10"](np.random.rand()) for i in range(len(ioc_detided))]
    )
    ioc_detided.plot.scatter(
        ax=ax,
        x="longitude",
        y="latitude",
        s=50,
...
def create_first_page(ioc_detided, gauges, area):
    ...
    tmean = avg_ts(
        [
            get_storm_peak_time(gauges, station)
            for station in ioc_detided.ioc_code.values
        ]
    )
                    ...
                    if not skill_regional.empty:
                        ...
                        for ip in range(0, Npages - 1):
                            fig = create_subsequent_pages(
                                ioc_detided, obs_root, gauges, ip * 8
                            )
                            pdf.savefig(fig)
                            plt.close(fig)
