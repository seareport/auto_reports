...
if pyproj.PROJ_VERSION >= (9, 6, 0):
    ...
    def wgs84_to_spilhaus(x, y):
        ...
        conformal_lat_center = -pi / 2 + 2 * arctan(
            ...
            * ((1 - e * sin(lat_center_rad)) / (1 + e * sin(lat_center_rad)))
            ** (e / 2),
        )
        alpha = -arcsin(cos(conformal_lat_center) * cos(azimuth_rad))
        lambda_0 = lon_center_rad + arctan2(
            tan(azimuth_rad),
            -sin(conformal_lat_center),
        )
        ...
        lat_s = (
            ...
            / pi
            * arcsin(
                sin(alpha) * sin(lat_c) - cos(alpha) * cos(lat_c) * cos(lon - lambda_0),
            )
        )
        lon_s = (
            180
            ...
            * (
                ...
                + arctan2(
                    ...
                    (
                        sin(alpha) * cos(lat_c) * cos(lon - lambda_0)
                        + cos(alpha) * sin(lat_c)
                    ),
                )
            )
