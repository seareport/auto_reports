...
def scatter_table(df, color):
    if df.empty:
        ...
        def create_plots(selected_stations, selected_storms):
            ...
            colors = make_brightness_gradient_hex(
                color,
                len(filtered_df.storm.unique()),
            )
            overlays = {
                storm: filtered_df[filtered_df.storm == storm]
                .hvplot.scatter(
                    x=KDIMS[0],
                    y=KDIMS[1],
                    c=colors[istorm],
            ...
            data_min = (
                min(filtered_df["observed"].min(), filtered_df["model"].min()) - 0.1
            )
            data_max = (
                max(filtered_df["observed"].max(), filtered_df["model"].max()) + 0.1
            )
